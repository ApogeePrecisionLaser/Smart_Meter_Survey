...
<script type="text/javascript" language="javascript">
    jQuery(function(){
        $("#searchRawWater").autocomplete("rawWaterLevelCont.do", {
            extraParams: {                
                action1: function() { return "getRawWater"}
            }
        });       
        $("#rawWaterName").autocomplete("rawWaterLevelCont.do", {
            extraParams: {
                action1: function() { return "getRawWater"}
            }
        });
    ...
    function fillColumns(id) {
        ...
        $("#rawWaterLevelId").val(document.getElementById(t1id + (lowerLimit + 0)).innerHTML);
        ...
        //$("#latitude").val(document.getElementById(t1id + (lowerLimit + 5)).innerHTML);
        //$("#longitude").val(document.getElementById(t1id + (lowerLimit + 6)).innerHTML);
    ...
    function displayList(){
        ...
        var level = $("#level").val();
        var url = "rawWaterLevelCont.do?"+queryString+"&searchRawWater="+searchRawWater;
    ...
    function makeEditable(id) {
        ...
        if(id == 'new') {
            ...
            document.getElementById("rawWaterLevelId").value='';
    ...
    function searchCheck()
    {
        ...
        if(button=="searchIn")
        {
            ...
            if(searchRawWater=="")
            {
                $("#searchMessage").html("<center>Please enter Raw Water name</center>");
                $("#searchRawWater").focus();
                return false;
            }
        }
    }
    ...
    function openMap(longitude, lattitude) {
                            ...
                            var url="rawWaterLevelCont.do?task=showMapWindow&logitude="+y+"&lattitude="+x;
...
<html>
    <head>
        ...
        <title>Raw Water Level Page</title>
    </head>
    <body>
                ...
                <DIV id="body" class="maindiv" align="center">
                    <table width="100%" align="center">
                        <tr><td>
                                <table align="center">
                                    <tr>
                                        <td align="center" class="header_table" width="100%"><b>Raw Water Level</b></td>
                                    </tr>
                                </table>
                            </td></tr>
                        <tr>
                            <td> <div align="center">
                                    <form name="form0" method="POST" action="rawWaterLevelCont.do" onsubmit="return searchCheck()">
                        ...
                        <%--  <c:if test="${isSelectPriv eq 'Y'}">--%>
                            <tr>
                                <td align="center">
                                    <form name="form1" method="POST" action="rawWaterLevelCont.do">
                                        <DIV class="content_div">
                                            <table id="table1" width="800"  border="1"  align="center" class="content">
                                                <tr >
                                                    <th class="heading">S.No.</th>
                                                    <th class="heading">Raw Water</th>
                                                    <th  class="heading">Level</th>
                                                    <th  class="heading">Date & Time</th>                                                    
                                                    <th  class="heading">Remark</th>                                                   
                                                </tr>
                                                <c:forEach var="rawWaterLevelBean" items="${requestScope['rawWaterLevelList']}"  varStatus="loopCounter">
                                                    <tr class="${loopCounter.index % 2 == 0 ? 'even': 'odd'}">
                                                        <td id="t1c${IDGenerator.uniqueID}" style="display: none;" onclick="fillColumns(id)">${rawWaterLevelBean.rawWaterLevelId}</td>
                                                        <td id="t1c${IDGenerator.uniqueID}" onclick="fillColumns(id)" align="center">${lowerLimit - noOfRowsTraversed + loopCounter.count}</td>
                                                        <td id="t1c${IDGenerator.uniqueID}"  onclick="fillColumns(id)" >${rawWaterLevelBean.rawWaterName}</td>
                                                        <td id="t1c${IDGenerator.uniqueID}"  onclick="fillColumns(id)" style="text-align: right;padding-right: 8px;">${rawWaterLevelBean.level}</td>
                                                        <td id="t1c${IDGenerator.uniqueID}"  onclick="fillColumns(id)" align="center">${rawWaterLevelBean.level_datetime}</td>
                                                        <td id="t1c${IDGenerator.uniqueID}"  onclick="fillColumns(id)">${rawWaterLevelBean.remark}</td>
                                                        <%--<td id="t1c${IDGenerator.uniqueID}"  onclick="fillColumns(id)">${rawWaterLevelBean.latitude}</td>
                                                        <td id="t1c${IDGenerator.uniqueID}"  onclick="fillColumns(id)">${rawWaterLevelBean.longitude}</td>
                                                        <td>
                                                            <input type="button" class="btn"  value ="View Map" id="map_container${loopCounter.count}" onclick="openMap('${rawWaterLevelBean.longitude}' , '${rawWaterLevelBean.latitude}');"/>
                                                        </td>--%>
                                                    </tr>
                                                </c:forEach>
                                                <tr>
                        ...
                        <tr>
                            <td align="center">
                                <div >
                                    <form name="form2" method="POST" action="rawWaterLevelCont.do" onsubmit="return verify()">
                                        <table id="table2"  class="content" border="0"  align="center" width="600">
                                            ...
                                            <tr>
                                                <th class="heading1">Raw Water
                                                    <input class="input" type="text" style="display:none;" id="rawWaterLevelId" name="rawWaterLevelId" value="" ></th>
                                                <td><input class="input" type="text" id="rawWaterName" name="rawWaterName" value="" size="40" disabled></td>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <input class="input" type="text" id="level_datetime" name="level_datetime" value="${level_datetime}" maxlength="10" size="12" disabled>
