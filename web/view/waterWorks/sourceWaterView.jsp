...
<script type="text/javascript" language="javascript">
    ...
    jQuery(function(){
        $("#searchSourceWater").autocomplete("sourceWaterCont.do", {
            extraParams: {                
                action1: function() { return "getSourceWater";}
            }
        });
        $("#cityName").autocomplete("sourceWaterCont.do", {
            extraParams: {
    ...
    function fillColumns(id) {
        ...
        $("#sourceWaterId").val(document.getElementById(t1id + (lowerLimit + 0)).innerHTML);
    ...
    function displayList(){
        ...
        var url = "sourceWaterCont.do?"+queryString+"&searchSourceWater="+searchSourceWater;
        popupwin = openPopUp(url, "Source Water Report", 500, 1000);
    }
    ...
    function makeEditable(id) {
        ...
        if(id == 'new') {
            ...
            document.getElementById("sourceWaterName").value='';
    ...
    function openMap(longitude, lattitude) {
                            ...
                            var url="sourceWaterCont.do?task=showMapWindow&logitude="+y+"&lattitude="+x;
                ...
                <DIV id="body" class="maindiv" align="center">
                    <table width="100%" align="center">
                        <tr><td>
                                <table align="center">
                                    <tr>
                                        <td align="center" class="header_table" width="100%"><b>Source Water</b></td>
                                    </tr>
                                </table>
                            </td></tr>
                        <tr>
                            <td> <div align="center">
                                    <form name="form0" method="POST" action="sourceWaterCont.do" onsubmit="return searchCheck()">
                        ...
                        <%-- <c:if test="${isSelectPriv eq 'Y'}">--%>
                            <tr>
                                <td align="center">
                                    <form name="form1" method="POST" action="sourceWaterCont.do">
                                        <DIV class="content_div">
                                            <table id="table1" width="800"  border="1"  align="center" class="content">
                                                <tr >
                                                    <th class="heading">S.No.</th>
                                                    <th class="heading">Source Water Name</th>
                                                ...
                                                <c:forEach var="sourceWaterBean" items="${requestScope['sourceWaterList']}"  varStatus="loopCounter">
                                                    <tr class="${loopCounter.index % 2 == 0 ? 'even': 'odd'}" >
                                                        <td id="t1c${IDGenerator.uniqueID}" style="display: none;" onclick="fillColumns(id)">${sourceWaterBean.sourceWaterId}</td>
                                                        <td id="t1c${IDGenerator.uniqueID}" onclick="fillColumns(id)" align="center">${lowerLimit - noOfRowsTraversed + loopCounter.count}</td>
                                                        <td id="t1c${IDGenerator.uniqueID}"  onclick="fillColumns(id)" >${sourceWaterBean.name}</td>
                                                        <td id="t1c${IDGenerator.uniqueID}"  onclick="fillColumns(id)" style="text-align: right;padding-right: 5px;">${sourceWaterBean.avgHeight}</td>
                                                        <td id="t1c${IDGenerator.uniqueID}"  onclick="fillColumns(id)" style="text-align: right;padding-right: 5px;">${sourceWaterBean.maxHeight}</td>
                                                        <td id="t1c${IDGenerator.uniqueID}"  onclick="fillColumns(id)" style="text-align: right;padding-right: 5px;">${sourceWaterBean.minHeight}</td>
                                                        <td id="t1c${IDGenerator.uniqueID}"  onclick="fillColumns(id)">${sourceWaterBean.cityName}</td>
                                                        <td id="t1c${IDGenerator.uniqueID}"  onclick="fillColumns(id)">${sourceWaterBean.address1}</td>
                                                        <td id="t1c${IDGenerator.uniqueID}"  onclick="fillColumns(id)">${sourceWaterBean.address2}</td>
                                                        <td id="t1c${IDGenerator.uniqueID}"  onclick="fillColumns(id)" align="center">${sourceWaterBean.dateTime}</td>
                                                        <td id="t1c${IDGenerator.uniqueID}"  onclick="fillColumns(id)">${sourceWaterBean.latitude}</td>
                                                        <td id="t1c${IDGenerator.uniqueID}"  onclick="fillColumns(id)">${sourceWaterBean.longitude}</td>
                                                        <td id="t1c${IDGenerator.uniqueID}"  onclick="fillColumns(id)">${sourceWaterBean.remark}</td>                                                        
                                                        <td>
                                                            <input type="button" class="btn"  value ="View Map" id="map_container${loopCounter.count}" onclick="openMap('${sourceWaterBean.longitude}' , '${sourceWaterBean.latitude}');"/>
                                                        </td>
                                                    </tr>
                                                </c:forEach>
                                                <tr>
                        ...
                        <tr>
                            <td align="center">
                                <div >
                                    <form name="form2" method="POST" action="sourceWaterCont.do" onsubmit="return verify()">
                                        <table id="table2"  class="content" border="0"  align="center" width="600">
                                            ...
                                            <tr>
                                                <th class="heading1" style="padding-left: 15px;">Source Water Name</th>
                                                <td>
                                                    <input class="input" type="text" id="sourceWaterName" name="sourceWaterName" value="" size="40" disabled>
                                                    <input class="input" type="text" style="display:none;" id="sourceWaterId" name="sourceWaterId" value="" >
                                                </td>                                            
